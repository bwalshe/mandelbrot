...
#ifdef DEBUG 
#define log_area(MSG, RB) (fprintf(stderr, "%s: %d, %d, %d, %d\n", \
				   MSG, \
				   RB->start_x, \
				   RB->start_y, \
                                   RB->end_x, \
...
mbt_rb_click_pressed (GtkGestureClick *gesture __attribute__((unused)),
                      int              n_press __attribute__((unused)),
                      double           widget_x,
                      double           widget_y,
                      MbtRubberBand    *self)
{
...
mbt_rubber_band_class_init (MbtRubberBandClass *class) {
  ...
  mbt_select_signal = g_signal_new ("selection-complete",
...
mbt_rubber_band_init (MbtRubberBand *band) {
  ...
  g_signal_connect (band->drag_gesture, "drag-end",
		    G_CALLBACK(mbt_rb_drag_end), band);
  gtk_gesture_single_set_exclusive (GTK_GESTURE_SINGLE (band->drag_gesture), TRUE);
  gtk_gesture_single_set_button (GTK_GESTURE_SINGLE (band->click_gesture), 0);
  gtk_gesture_single_set_exclusive (GTK_GESTURE_SINGLE (band->click_gesture), TRUE);
